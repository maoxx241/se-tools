...
summary.getRange('G6:H17').setNumberFormat('#,##0'); summary.getRange('I6:L17').setNumberFormat('#,##0.000');
summary.getRange('M1:M65').format.columnWidth=33;
value(summary,'A20','配套 KV 存取 · 每请求 / 每 P DP 批 / P 整组'); summary.getRange('A20').format.font={size:14,bold:true};
value(summary,'A21','单位 GiB。存＝生成 KV 行＋最终辅助状态；取＝P→D 张量字节。V4.1 的取仅为规划值，main 未支持迁移。');
header(summary,23,['模型','EP','P TP','P DP','D TP','D DP','存/请求','取或规划取/请求','存/P DP批','取或规划取/批','存/P整组','取或规划取/整组','部署 / 迁移条件']);
summary.getRange('G24:L35').setNumberFormat('#,##0.000');
value(summary,'A38','EP256 条件：DS V4 / V4.1 / Kimi 需补齐 128 个物理专家槽并验证 EPLB；未改模型 checkpoint。');
formula(summary,'A40','"P 整组批次：EP32 为 "&\'KV明细\'!$K$5*4&" 请求，EP256 为 "&\'KV明细\'!$K$5*32&" 请求；单请求 KV 不随 EP 改变。"');
value(summary,'A42','DS 存量含整个 prompt 生成的 SWA 行；阶段末常驻量见 KV 明细，不能按生成量配 HBM。');
value(summary,'A44','V4.1：完成 S tokens 后交接；长 KV / Indexer 只计 4 个源层，SWA 尾块和 32 行环形页按规划传输。');
value(summary,'A46','MC2 未计轮询读、重试和物理协议开销；V4.1 Engram 通信另见模型分析，未叠加进 MoE EP。');
value(summary,'A48','黄色输入可编辑；主干层，不含 draft/MTP；参数、公式与源码说明见明细及 analysis/KV-EP-SPECS.md。');
...
header(kv,8,['模型','缓存组件','层数','每行 B/Rank','P 生成行','P 保留行','D 取/规划行','P 副本/分片','D 副本/分片','存 GiB','保留 GiB','取/规划 GiB']);
...
for(const item of cases.filter(x=>x.ep===32)) {
  ...
  for(const p of item.cache.components) {
    const v41=item.profile==='deepseek_v41';
    const N=item.profile==='glm53'||v41?'$B$5':'($B$5-1)', B='$E$5';
    let g, keep, pull;
    if(p.component.startsWith('SWA')) {
      const W=Number(p.retainedRows); g=N; keep=`MIN(${N},${W})`;
      pull=`MIN(ROUNDUP(${N}/${B},0),ROUNDUP(${W}/${B},0)+1)*${B}`;
    } else if(p.component.includes('circular ring')) {
      g=keep=`MIN(${N},32)`; pull='32';
      ...
      g=keep=`INT(${N}/${ratio})`;
      pull=v41?`ROUNDUP(INT(${N}/${ratio})/(${B}/${ratio}),0)*(${B}/${ratio})`:`ROUNDUP(${N}/(${ratio}*${B}),0)*${B}`;
    }
    ...
    if(v41) value(kv,`M${row}`,'迁移未支持；取为规划值');
...
kv.getRange('M1:M65').format.columnWidth=30;
value(kv,`A${row+1}`,'前五模型：MooncakeConnector；P/D 同 block size、同 speculative 配置；无前缀命中、PCP/DCP=1。');
value(kv,`A${row+3}`,'GLM: 78 主干 / 21 Indexer；Kimi: 24 MLA + 69 KDA；Qwen: 23 GQA + 69 GDN。');
value(kv,`A${row+5}`,'V4.1：S tokens 快照；C2 物理页 B/2 行，只计完成的压缩对；环形 state 规划传完整 32 行页。');
value(kv,`A${row+7}`,'来源：归档 config / VA 842b030f / vLLM c8438a3；V4.1 main 1933f86；路径/哈希见源码清单。');
...
ep.getRange('C24:K35').setNumberFormat('#,##0.000000'); ep.freezePanes.freezeRows(9);
for(let i=0;i<cases.length;i++) {
  const r=cases[i], a=10+i,b=24+i,s=24+i,e=6+i,k=totals.get(r.slug);
  ...
  if(r.profile==='deepseek_v41') {
    formula(summary,`M${s}`,`IF('EP通信'!H${a}>0,"补128槽并验证；取为规划值","取为规划值；迁移未支持")`);
    summary.getRange(`H${s}`).format.font.color='#955700';
    summary.getRange(`J${s}`).format.font.color='#955700';
    summary.getRange(`L${s}`).format.font.color='#955700';
  }
  summary.getRange(`A${e}:M${e}`).formulas=[[
    ...['A','B','C','D','E','F'].map(c=>`=${c}${s}`),`='EP通信'!K${a}*B${e}`,`='EP通信'!L${a}*B${e}`,
    `='EP通信'!F${b}`,`='EP通信'!K${b}`,`=I${e}/B${e}`,`=J${e}/B${e}`,`=M${s}`]];
}
ep.getRange('H10:H21').conditionalFormats.add('cellIs',{operator:'greaterThan',formula:0,format:{fill:'#FFF0C2',font:{color:'#955700',bold:true}}});
...
function verify(expected) {
  for(let i=0;i<expected.length;i++) {
    const r=expected[i],n=24+i,m=6+i;
    const cells=[[`G${n}`,r.cache.generatedBytes],[`H${n}`,pullEstimate(r.cache)],[`I${n}`,r.batch.generatedBytes],[`J${n}`,pullEstimate(r.batch)],
      [`K${n}`,r.wholePrefillGroup.generatedBytes],[`L${n}`,pullEstimate(r.wholePrefillGroup)],[`I${m}`,r.prefillEP.modeledBytes],[`J${m}`,r.decodeEP.modeledRemoteWriteBytes]];
    ...
    assert(Math.abs(kv.getRange(`K${totals.get(r.slug)}`).values[0][0]*2**30-Number(r.cache.retainedBytes))<1);
  }
}
...
console.log(`Verified ${cases.length} cases against CLI, input mutations and 3 sheet renders; exported kv-ep32-ep256.xlsx`);
