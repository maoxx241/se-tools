...
import { decodeTpTraffic } from './communication-requirements.mjs';
...
export const MINUTE_DEFAULTS = Object.freeze({ requestsPerDP: 16, prefillGroups: 1,
  ...
  tpBandwidthGBps: 4800, tpSharedFraction: 1, tpExposedFraction: 1 });

// Uniformly placed equal-length layer communication blocks. The complement consists of
...
export function minuteImpact({ pullBytesPerRequest, ep, prefillTP, decodeTP,
  decodeBytesPerRank, decodeTpBytesPerRank = 0, layers, ttftSeconds, tpotMs, ...options }) {
  const o = { ...MINUTE_DEFAULTS, ...options };
  for (const [k,v] of Object.entries({ pullBytesPerRequest, ep, prefillTP, decodeTP,
    decodeBytesPerRank, decodeTpBytesPerRank, layers, ttftSeconds, tpotMs, ...o })) {
  ...
  for (const k of ['kvBandwidthGBps','decodeBandwidthGBps','tpBandwidthGBps','outputTokensPerStep'])
    if (o[k] <= 0) throw new Error(`${k} must be positive`);
  for (const k of ['bandwidthLoss','sharedFraction','exposedFraction','tpSharedFraction','tpExposedFraction'])
    if (o[k]<0 || o[k]>1 || (k==='bandwidthLoss' && o[k]===1)) throw new Error(`Invalid ${k}`);
  if (pullBytesPerRequest < 0 || decodeBytesPerRank < 0 || decodeTpBytesPerRank < 0 || ttftSeconds <= 0 || tpotMs <= 0
  ...
  const tpCommMs=decodeTpBytesPerRank/(o.tpBandwidthGBps*1e6), tpLayerCommMs=tpCommMs/layers;
  const totalCommMs=commMs+tpCommMs, combinedLayerCommMs=totalCommMs/layers;
  let status='ok';
  if (windowMs>=periodMs) status='kv-overload';
  else if (totalCommMs>stepMs+1e-9) status='tpot-infeasible';
  else if (stepMs>periodMs || Math.max(layerCommMs,tpLayerCommMs)>periodMs-windowMs) status='timeline-required';
  ...
  const result={status, pDP, dDP, periodMs, stepMs, windowMs,
    ...
    offeredKVUtilization:windowMs/periodMs, commMs, layerCommMs, tpCommMs, tpLayerCommMs, totalCommMs,
    minimumDecodeBandwidthGBps:stepMs>tpCommMs ? decodeBytesPerRank/((stepMs-tpCommMs)*1e6) : null, baselineStepsPerMinute,
    overlappingStepsPerMinute:null, allEPInsideStepsPerMinute:null, partialStepsPerMinute:null,
    allCommunicationInsideStepsPerMinute:null, extraEpMsForBaselineMinute:null, extraTpMsForBaselineMinute:null,
    ...
    decodeThroughputLossFraction:null };
  if (status!=='ok') return result;
  // One combined block per equivalent layer, with adjacent EP and TP segments.
  // Boundary TP events are amortized over these blocks; this is not a trace.
  const any=overlapProbability(windowMs,periodMs,stepMs,totalCommMs,layers);
  const all=totalCommMs===0 ? 0 : Math.max(0,1-overlapProbability(periodMs-windowMs,periodMs,stepMs,totalCommMs,layers));
  const allEP=commMs===0 ? 0 : Math.max(0,1-overlapProbability(periodMs-windowMs,periodMs,stepMs,commMs,layers));
  const extraEP=baselineStepsPerMinute*layers*meanEventDelay(layerCommMs,windowMs,periodMs,o.bandwidthLoss)
    *o.sharedFraction*o.exposedFraction;
  const extraTP=baselineStepsPerMinute*layers*meanEventDelay(tpLayerCommMs,windowMs,periodMs,o.bandwidthLoss)
    *o.tpSharedFraction*o.tpExposedFraction;
  const extra=extraEP+extraTP;
  return {...result,
    overlappingStepsPerMinute:baselineStepsPerMinute*any,
    allEPInsideStepsPerMinute:baselineStepsPerMinute*allEP,
    allCommunicationInsideStepsPerMinute:baselineStepsPerMinute*all,
    extraEpMsForBaselineMinute:extraEP, extraTpMsForBaselineMinute:extraTP,
    partialStepsPerMinute:baselineStepsPerMinute*Math.max(0,any-all),
    overlappingLayerCallsPerMinute:totalCommMs===0||windowMs===0 ? 0 : baselineStepsPerMinute*layers*Math.min(1,(windowMs+combinedLayerCommMs)/periodMs),
    ...
    // Same active batch and accepted tokens per step, with Decode kept busy.
    decodeThroughputLossFraction:extra/(60000+extra) };
}
  ...
  for (const scenario of cases) {
    for (const r of await loadKvEpSpecs({promptTokens:scenario.promptTokens,
      ...
      for (const tpotMs of tpots) {
        const tpTraffic=decodeTpTraffic(r.facts,o.decodeTokensPerDP,r.topology.decodeTP,
          {sharedExpertTpEnabled:r.profile==='kimi'});
        const inputs={...o,pullBytesPerRequest:Number(r.cache.pullBytes??r.cache.plannedPullBytes),
          ...
          decodeTpBytesPerRank:tpTraffic.totalBytesPerRank,
        ...
        rows.push({model:r.model,slug:r.slug,profile:r.profile,promptTokens:scenario.promptTokens,
          ...
          tpTraffic,inputs,result:minuteImpact(inputs)});
      }
    }
  }
  return rows;
