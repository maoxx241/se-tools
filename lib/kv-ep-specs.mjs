...
export function kvComponents(model, config, options = {}) {
  ...
  const ds = model.profile === 'deepseek_v4', glm = model.profile === 'glm53';
  const DTP = ds || glm ? 1n : 8n;
  const N = glm ? S : S - 1n; // Mooncake truncates compressed/state-model prefill.
  const parts = [];
  function add(component, layers, rowBytes, generatedRows, retainedRows, pullRows, pCopies = TP, dCopies = DTP, kind = 'kv') {
    ...
    r.pullBytes = r.layers * r.rowBytes * pullRows * dCopies;
    parts.push(r);
  }
  ...
  return { prefillTokens: N, prefillTP: TP, decodeTP: DTP, components: parts,
    ...
    pullBytes: parts.reduce((s, p) => s + p.pullBytes, 0n),
...
export async function loadKvEpSpecs(options = {}) {
  ...
  for (const model of communicationModels.slice(0, 5)) {
    ...
    for (const ep of [32, 256]) {
      ...
      rows.push({ model: model.name, slug: model.slug, profile: model.profile, ep, facts, options: o,
        ...
        cache, batch: { requests: R, generatedBytes: cache.generatedBytes * R, retainedBytes: cache.retainedBytes * R, pullBytes: cache.pullBytes * R },
        wholePrefillGroup: { requests: R * pDP, generatedBytes: cache.generatedBytes * R * pDP, pullBytes: cache.pullBytes * R * pDP },
